...
def extract_family_name(full_name):
    ...
    semicolon_index = full_name.index("; ") # space is the mistake here
...
def extract_given_name(full_name):
    ...
    """
    # Find the index where "; " appears within the full name string.
    semicolon_index = full_name.index("; ") # it's not / but ;

    # Extract a substring from the full name and return it.
    given_name = full_name[semicolon_index + 2 : ] # need to use + 1 otherwise it will eat up the first character of the first name, leaving blank after : gets everything in the string left after ;
    return given_name
